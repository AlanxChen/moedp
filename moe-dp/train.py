...
from moe_dp.dataset.base_dataset import BaseDataset
from moe_dp.env_runner.base_runner import BaseRunner
from moe_dp.common.checkpoint_util import TopKCheckpointManager
from moe_dp.common.pytorch_util import dict_apply, optimizer_to
from moe_dp.model.diffusion.ema_model import EMAModel
from moe_dp.model.common.lr_scheduler import get_scheduler
...
OmegaConf.register_new_resolver("split_before_underscore", lambda x: x.split("_")[0])
OmegaConf.register_new_resolver("capitalize", lambda x: x.capitalize())
...
def _resolve_cfg_paths(cfg: OmegaConf):
    if "task" in cfg and "dataset" in cfg.task and "dataset_path" in cfg.task.dataset:
        cfg.task.dataset.dataset_path = str(_resolve_repo_path(cfg.task.dataset.dataset_path))
...
class TrainWorkspace:
    ...
    def run(self):
        ...
        env_runner = hydra.utils.instantiate(
            cfg.task.env_runner,
            output_dir=self.output_dir,exp_name=cfg.exp_name)
        ...
        if cfg.training.max_train_steps is not None:
            max_train_batches = min(max_train_batches, cfg.training.max_train_steps)
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                ...
                for batch_idx, batch in enumerate(tepoch):
                    ...
                    if 'task_step' in batch['obs']:
                        batch['obs']['task_step'] = batch['obs']['task_step'].long()
                    ...
                    is_epoch_end_batch = (batch_idx + 1) == max_train_batches
                    should_step = ((batch_idx + 1) % gradient_accumulate_every == 0) or is_epoch_end_batch
                    ...
                    if should_step and cfg.training.use_ema:
                        ema.step(self.model)
                    t1_4 = time.time()
                    ...
                    if should_step:
                        ...
                        if not is_epoch_end_batch:
                            # The final optimizer step is logged together with validation and rollout.
