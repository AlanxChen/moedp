...
class AsyncVectorEnv(VectorEnv):
    ...
    def get_reset_obj_position(self):
        ...
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError(
                "Calling `get_reset_obj_position` while waiting "
                f"for a pending call to `{self._state.value}` to complete.",
                self._state.value,
            )

        for pipe in self.parent_pipes:
            pipe.send(("get_reset_obj_position", None))
        _, successes = zip(*[pipe.recv() for pipe in self.parent_pipes])
        self._raise_if_errors(successes)


    def reset_obj(self, task_step_list):
        self._assert_is_running()
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError(
                "Calling `reset_obj` while waiting "
                f"for a pending call to `{self._state.value}` to complete.",
                self._state.value,
            )

        for pipe, task_step in zip(self.parent_pipes,task_step_list):
            pipe.send(("reset_obj", task_step))
        _, successes = zip(*[pipe.recv() for pipe in self.parent_pipes])
        self._raise_if_errors(successes)
...
def _worker_shared_memory(index, env_fn, pipe, parent_pipe, shared_memory, error_queue):
    ...
    try:
        while True:
            ...
            elif command == "get_reset_obj_position":
                env.get_reset_obj_position()
                pipe.send((None, True))
            elif command == "reset_obj":
                env.reset_obj(data)
                pipe.send((None, True))
            else:
                raise RuntimeError(
