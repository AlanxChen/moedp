...
from moe_dp.model.common.dict_of_tensor_mixin import DictOfTensorMixin
from PIL import Image

class LinearNormalizer(DictOfTensorMixin):
    avaliable_modes = ['limits', 'gaussian']
    exclude_keys = ["img",'time',"norm_image","task_step","render_image","agentview_rgb","eye_in_hand_rgb","render_hand_image"]
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                if key in self.exclude_keys:
                    # Keep explicitly excluded keys unchanged.
                    continue
